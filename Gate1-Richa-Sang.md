# Gate 1 — AI-Native Specification
...
Acknowledgment is a templated communication triggered by state transition. The content is parameterised (reference number, claim type, assigned adjuster name for non-escalated claims, expected next-contact window). There is no judgment involved. Sending a templated acknowledgment within minutes of receipt is a capability gap today only because humans are doing higher-value work simultaneously — an agent can do this instantly and correctly every time.
...
The happy path runs straight down. Any step can branch right into the Human Review Queue; the human decision re-enters the pipeline at routing.
...
flowchart TD
    ...
    Send receipt acknowledgment immediately"]
...
The agent is the central orchestrator. It reads from intake channels, calls three external systems, and produces two outputs: a claimant acknowledgment and an adjuster assignment. The Human Review Queue is managed inside the CRM and feeds decisions back to the agent.
...
ExtractedData {
  ...
  extraction_confidence: float        # 0.0–1.0; agent-assigned
...
| State | Description |
|---|---|
| `RECEIVED` | Raw claim persisted to CRM; reference number assigned |
...
| `ROUTED` | Adjuster assigned; acknowledgment pending |
| `ACKNOWLEDGED` | Claimant notified; claim on automated path done |
...
The `FNOLOrchestrator` agent has access to the following tools:
...
def parse_claim_document(claim_id: str, raw_content: str, source: str) -> ExtractedData:
    """Calls LLM extraction with structured output schema. Returns ExtractedData with confidence score."""
...
def classify_severity(extracted: ExtractedData, policy: PolicyResult) -> TriageResult:
    """Applies deterministic rule set (see §3.4). Returns severity + confidence."""
...
def send_acknowledgment(claim_id: str, claimant_contact: dict, template_id: str, template_data: dict) -> AcknowledgmentResult:
    """Sends via CRM communications endpoint. Channel priority: email > sms > portal."""

def escalate_to_human(claim_id: str, reason_code: str, dossier: dict, priority: Literal["urgent","standard"]) -> None:
    """Creates CRM review task. Priority = urgent if severity HIGH or SLA < 45 minutes remaining."""
...
Run a LOW-severity email claim through the full pipeline. Verify: state transitions fire in order, CRM records update correctly, acknowledgment is sent, COMPLETE state is reached. Measure wall-clock time: target < 60 seconds for fully automated claims.

**Integration: Escalation path**
Run a HIGH-severity claim and verify: claim enters ESCALATED, human review task is created in CRM with correct dossier, human action transitions claim back to pipeline correctly.
...
| Test type | Extraction | Policy lookup | Severity | Routing | Escalation | SLA timing | Error recovery |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
...
The CRM fallback path assumes claims can be persisted locally when CRM is unavailable. If the deployment environment has no Redis, database, or queue infrastructure, the fallback must be redesigned. Given the client has "no AI infrastructure today," this must be provisioned.
